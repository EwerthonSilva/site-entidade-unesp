...
function edShowButton(which, button, i) {
	...
	switch (button.id) {
		case 'ed_img':
			document.write('<span  id="' + button.id + '_' + which + '" ' + accesskey + ' class="ed_button" onclick="edInsertImage(\'' + which + '\');"   >' + button.display + '</span>');
			break;
		case 'ed_link':
			document.write('<span  id="' + button.id + '_' + which + '" ' + accesskey + ' class="ed_button" onclick="edInsertLink(\'' + which + '\', ' + i + ');"   >' + button.display + '</span>');
			break;
		case 'ed_ext_link':
			document.write('<span  id="' + button.id + '_' + which + '" ' + accesskey + ' class="ed_button" onclick="edInsertExtLink(\'' + which + '\', ' + i + ');"   >' + button.display + '</span>');
			break;
		case 'ed_footnote':
			document.write('<span  id="' + button.id + '_' + which + '" ' + accesskey + ' class="ed_button" onclick="edInsertFootnote(\'' + which + '\');"   >' + button.display + '</span>');
			break;
		case 'ed_via':
			document.write('<span  id="' + button.id + '_' + which + '" ' + accesskey + ' class="ed_button" onclick="edInsertVia(\'' + which + '\');"   >' + button.display + '</span>');
			break;
		default:
			document.write('<span  id="' + button.id + '_' + which + '" ' + accesskey + ' class="ed_button" onclick="edInsertTag(\'' + which + '\', ' + i + ');"    >' + button.display + '</span>');
			break;
	}
}
...
function edAddTag(which, button) {
	if (edButtons[button].tagEnd != '') {
		...
		document.getElementById(edButtons[button].id + '_' + which).innerHTML = '/' + document.getElementById(edButtons[button].id + '_' + which).innerHTML;
	}
}
...
function edRemoveTag(which, button) {
	for (i = 0; i < edOpenTags[which].length; i++) {
		if (edOpenTags[which][i] == button) {
			...
			document.getElementById(edButtons[button].id + '_' + which).innerHTML = document.getElementById(edButtons[button].id + '_' + which).innerHTML.replace('/', '');
		}
	}
}
...
function edToolbar(which, params) {
	document.write('<div id="ed_toolbar_' + which + '" class="quicktags-toolbar '+ (params.classes ? params.classes : '') +'" style="'+(params.styles ? params.styles : '')+'"><span>');
	for (i = 0; i < extendedStart; i++) {
		edShowButton(which, edButtons[i], i);
	}
	if (edShowExtraCookie()) {
		document.write(
			'<span  id="ed_close_' + which + '" class="ed_button" onclick="edCloseAllTags(\'' + which + '\');"  >fechar tags</span>'
			+ '<span  id="ed_spell_' + which + '" class="ed_button" onclick="edSpell(\'' + which + '\');"   >Dict</span>'
			+ '<span  id="ed_extra_show_' + which + '" class="ed_button" onclick="edShowExtra(\'' + which + '\')" style="visibility: hidden;" >&raquo;</span>'
			+ '</span><br />'
			+ '<span id="ed_extra_buttons_' + which + '">'
			+ '<span  id="ed_extra_hide_' + which + '" class="ed_button" onclick="edHideExtra(\'' + which + '\');"   >&laquo;</span>'
		);
	}
	else {
		document.write(
			'<span  id="ed_close_' + which + '" class="ed_button" onclick="edCloseAllTags(\'' + which + '\');"   >fechar tags</span>'
			//+ '<span  id="ed_spell_' + which + '" class="ed_button" onclick="edSpell(\'' + which + '\');"   >Dict</span>'
			+ '<span  id="ed_extra_show_' + which + '" class="ed_button" onclick="edShowExtra(\'' + which + '\')"   >&raquo;</span>'
			+ '</span><br />'
